...
class DocumentRepository:
    ...
    def create(self, application_id: str, doc_type: str, file_ref: str,
               upload_channel: str = "WEB", upload_source_ref: str = None) -> Document:
        doc = Document(
    ...
    def update_ocr_result(self, doc_id: str, extracted_fields: dict,
                          confidence_score: float) -> Document | None:
        doc = self.get(doc_id)
        if doc:
    ...
    def update_match_scores(self, doc_id: str, field_match_scores: dict,
                            overall_match_score: float, mismatch_fields: list) -> Document | None:
        doc = self.get(doc_id)
        if doc:
            doc.field_match_scores = field_match_scores
            doc.overall_match_score = overall_match_score
            doc.mismatch_fields = mismatch_fields
            doc.verification_status = "REVIEW_REQUIRED" if mismatch_fields else "MATCHED"
            doc.updated_at = datetime.datetime.utcnow()
            self.db.commit()
        return doc
